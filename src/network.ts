...
import type { Action } from './scrabble';
...
type WireMessage =
  ...
  | { type: 'welcome'; members: RoomMember[]; started: boolean }
  | { type: 'lobby'; members: RoomMember[]; started: boolean }
...
export type RoomCallbacks = {
  ...
  members?: (members: RoomMember[], started: boolean) => void;
...
export class ScrabbleRoom {
  ...
  private constructor(code: string, role: 'host' | 'guest', token: string, name: string, callbacks: RoomCallbacks) {
    this.code = code;
    this.role = role;
    this.token = token;
    this.name = name;
  ...
  static createHost(name: string, code = safeCode(), callbacks: RoomCallbacks = {}) {
    return new ScrabbleRoom(code, 'host', randomToken(), name.trim() || 'Host', callbacks);
  }

  static join(code: string, name: string, token = randomToken(), callbacks: RoomCallbacks = {}) {
    return new ScrabbleRoom(code.trim().toUpperCase(), 'guest', token, name.trim() || 'Player', callbacks);
  }
  ...
  private bindPeer() {
    ...
    this.peer.on('open', () => {
      if (this.role === 'host') {
        ...
        this.callbacks.members?.(this.members, this.started);
      } else {
    ...
    this.peer.on('disconnected', () => this.report('closed', 'The signaling server disconnected.'));
  ...
  private bindConnection(connection: DataConnection, outgoing: boolean) {
    connection.on('open', () => {
      ...
      if (this.role === 'host' && !outgoing) {
        ...
        this.send(connection, { type: 'welcome', members: this.members, started: this.started });
        this.broadcastLobby();
        this.callbacks.members?.(this.members, this.started);
    ...
    connection.on('error', error => this.callbacks.error?.(error instanceof Error ? error.message : String(error)));
  }
  ...
  private broadcastLobby() {
    const message: WireMessage = { type: 'lobby', members: this.members, started: this.started };
  ...
  start() {
    ...
    this.started = true;
    this.broadcastLobby();
    this.callbacks.members?.(this.members, true);
  }
